...
export default function App() {
  ...
  const interactWithStation = (stationId: string) => {
    setStations(prev => {
      ...
      // Cooked/Prepped Auto-Placing Logic
      if ((station.type === 'STOVE' || station.type === 'PREP') && station.content && !Array.isArray(station.content)) {
        const ingredient = station.content as Ingredient;
      ...
      // Interaction Logic (Held Item to Station)
      if (heldItem && !Array.isArray(heldItem)) {
      ...
      // Pick up logic
        ...
        if (station.type === 'PLATE' && Array.isArray(station.content) && station.content.length > 0) {
          setHeldItem(station.content);
          newStations[index] = { ...station, content: [] };
        } else if (!Array.isArray(station.content)) {
          setHeldItem(station.content);
          newStations[index] = { ...station, content: null };
        }
      }
  ...
  return (
    <div className="h-full w-full flex items-center justify-center font-sans overflow-hidden bg-stone-200 relative">
      <div className="relative w-full max-w-[430px] h-full bg-transparent overflow-hidden">
        ...
        <header className="h-14 bg-[#8b4f2f]/95 flex items-center justify-between px-4 z-50 relative shadow-md">
          <div className="flex items-center gap-2"><ChefHat className="text-amber-100 w-5 h-5"/><span className="text-amber-50 font-bold">{currentLevel.name}</span></div>
          <div className="flex gap-2 text-xs font-bold">
            <div className="bg-black/20 px-2 py-1 rounded text-amber-50">⏱ {Math.floor(timeLeft / 60)}:{(timeLeft % 60).toString().padStart(2, '0')}</div>
            <div className="bg-black/20 px-2 py-1 rounded text-amber-50">💰 {coins}</div>
          </div>
        </header>

        <main className="h-[calc(100vh-56px)] relative z-0">
          {/* Customers */}
          <div className="absolute left-0 right-0 bottom-[52%] h-[200px] flex justify-between px-4 items-end z-10">
            {[0, 1, 2].map(slot => {
              const customer = customers.find(c => c.slotIndex === slot);
              return (
                <div key={slot} className="w-[120px] relative flex flex-col items-center">
                  <AnimatePresence>
                    {customer && (
                      <motion.div initial={{y:20, opacity:0}} animate={{y:0, opacity:1}} exit={{y:20, opacity:0}} onClick={() => serveCustomer(slot)}>
                        <div className="absolute -top-16 left-1/2 -translate-x-1/2 z-40 bg-white p-1 rounded-xl shadow-xl border-2 border-stone-100">
                          <img src={RECIPES[customer.order!.recipeId].image} className="w-10 h-10 object-contain"/>
                        </div>
                        <img src={customer.image} className="w-24 h-24 object-contain drop-shadow-lg"/>
                      </motion.div>
                    )}
                  </AnimatePresence>
                </div>
          ...
          <div className="absolute left-0 right-0 bottom-0 z-20"><img src={counterWoodImage} className="w-full object-cover"/></div>

          {/* STATIONS AREA: 核心修改点 */}
          <div className="absolute left-0 right-0 bottom-[20%] px-4 z-30">
              ...
              {stations.map(station => (
                ...
                >
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <img 
                      src={
                        station.id.startsWith("grill") ? stationGrillImage : 
                        station.id.startsWith("fryer") ? stationFryerImage : 
                        station.id.startsWith("prep") ? stationSauceImage : plateImage
                      } 
                      className="w-full h-full object-contain opacity-100 transition-transform group-active:scale-95"
                    />
                  </div>
                  ...
                  <div className="relative z-10 flex flex-col items-center justify-center w-full h-full pointer-events-none">
                    {station.content && (
                      Array.isArray(station.content) ? (
                        /* 盘子里的多食材堆叠 */
                        <div className="flex flex-wrap gap-0.5 justify-center scale-75">
                          {(station.content as Ingredient[]).map((ing, i) => (
                            <img key={i} src={INGREDIENT_DATA[ing.id].image} className="w-6 h-6 object-contain drop-shadow-sm" />
                          ))}
                        </div>
                      ) : (
                        /* 单个烹饪中的食材 */
                        <div className="flex flex-col items-center">
                          ...
                          {(station.content as Ingredient).state === IngredientState.COOKING && (
                            <div className="w-12 h-1.5 bg-black/40 rounded-full overflow-hidden mt-1">
                          ...
                          {(station.content as Ingredient).state === IngredientState.COOKED && (
                            <CheckCircle2 className="w-5 h-5 text-emerald-500 mt-1 drop-shadow-sm" />
                          )}
                        </div>
                      )
                    )}
          ...
          {/* Bottom Ingredients */}
          <div className="absolute left-0 right-0 bottom-6 px-4 flex justify-between items-center z-50">
            <div className="flex gap-2 overflow-x-auto pb-2">
              {currentLevel.unlockedIngredients.map(id => (
                <div key={id} onClick={() => takeIngredient(id)} className="w-16 h-20 bg-white/90 rounded-xl flex flex-col items-center justify-center shadow-sm border border-stone-200 shrink-0">
                  <img src={INGREDIENT_DATA[id].image} className="w-10 h-10 object-contain mb-1" />
                  <span className="text-[9px] font-bold text-stone-500 uppercase">{INGREDIENT_DATA[id].name}</span>
                </div>
              ))}
            </div>
            <div onClick={() => setHeldItem(null)} className="w-14 h-14 bg-red-100 rounded-full flex items-center justify-center border-2 border-red-200 cursor-pointer shadow-lg active:scale-90 transition-transform">
              <XCircle className="text-red-500 w-8 h-8" />
            </div>
          </div>
        </main>

        {/* Level Select & Overlays */}
        {gameState === 'START' && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-6">
            <div className="bg-white rounded-3xl p-10 w-full text-center shadow-2xl">
              <ChefHat className="w-16 h-16 text-emerald-600 mx-auto mb-4"/>
              <h1 className="text-3xl font-bold mb-2">Kitchen Master</h1>
              <p className="text-stone-500 mb-8">Ready to cook some delicious food?</p>
              <button onClick={() => setGameState('LEVEL_SELECT')} className="w-full bg-emerald-600 text-white font-bold py-4 rounded-2xl shadow-lg">START COOKING</button>
            </div>
          </div>
        )}
        ...
        {gameState === 'LEVEL_SELECT' && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-6">
            <div className="bg-white rounded-3xl p-6 w-full max-h-[70vh] overflow-y-auto">
              <h2 className="text-xl font-bold mb-4 text-center">Select Level</h2>
              <div className="grid grid-cols-4 gap-3">
                {LEVELS.map((level, i) => (
                  <button key={i} onClick={() => startLevel(i)} className="aspect-square bg-stone-100 rounded-xl font-bold text-lg hover:bg-emerald-100 transition-colors">{level.id}</button>
                ))}
              </div>
            </div>
          </div>
...
{/* Main Game Area: 改为 relative 容器，去掉所有 flex 约束 */}
      <main className="h-[calc(100vh-56px)] relative z-0 overflow-hidden">
        ...
        <div className="absolute left-0 right-0 bottom-[35%] h-[200px] flex justify-between px-4 items-end z-10 pointer-events-none">
          {[0, 1, 2].map(slot => {
            const customer = customers.find(c => c.slotIndex === slot);
            return (
